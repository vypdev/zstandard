...
Future<void> main(List<String> args) async {
  ...
  if (!_packages.contains(package) ||
    ...
    stderr.writeln('Package or stable version is not part of this release.');
    exitCode = 2;
    return;
  }
  ...
  try {
    ...
    if (response.statusCode != HttpStatus.ok) {
      stderr.writeln(
        'pub.dev returned HTTP ${response.statusCode} for $package.',
      );
      exitCode = 1;
      return;
    }
  ...
  stdout.write(dryRun.stdout);
  stderr.write(dryRun.stderr);
  if (dryRun.exitCode != 0) {
    stderr.writeln(
      'Dry-run validation failed; $package $version was not published.',
    );

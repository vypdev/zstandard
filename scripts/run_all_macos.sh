#!/usr/bin/env bash
# Run all preparation, build, and test steps that are runnable on macOS.
# Usage: from repo root, run: ./scripts/run_all_macos.sh
# Set ZSTANDARD_APPLE_DEPENDENCY_MANAGER=cocoapods to exercise the CocoaPods
# compatibility path instead of the default Swift Package Manager path.
#
# Steps:
...
#
# Requires: macOS, Flutter 3.44+, Dart SDK, Xcode, Android SDK/NDK (for Android),
#           CMake (for CLI build). CocoaPods is required only for compatibility mode.
...
APPLE_DEPENDENCY_MANAGER="${ZSTANDARD_APPLE_DEPENDENCY_MANAGER:-swiftpm}"
...
# 3. Build Android (the plugin is built through the example app)
step "3/12 — Build Android"
cd "$EXAMPLE" && flutter build apk && cd "$ROOT"

# 4. Build CLI (macOS native libraries for zstandard_cli)
...
cd "$EXAMPLE" && flutter pub get && cd "$ROOT"
if [[ "$APPLE_DEPENDENCY_MANAGER" == "cocoapods" ]]; then
...
if [[ "$APPLE_DEPENDENCY_MANAGER" == "cocoapods" ]]; then
  cd "$EXAMPLE/macos" && pod install && cd "$ROOT"
fi

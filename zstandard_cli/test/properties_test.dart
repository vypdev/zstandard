...
void main() {
  ...
  group('Property-based tests', () {
    property('roundtrip: decompress(compress(x)) == x', () {
      forAll(binary(minLength: 0, maxLength: 2000), (List<int> data) async {
      ...
      () {
        forAll(
          ...
          (tuple) async {
            ...
            final input = Uint8List.fromList(data);
            final cli = ZstandardCLI();
            final compressed1 = await cli.compress(
              input,
              compressionLevel: level,
            );
            final compressed2 = await cli.compress(
              input,
              compressionLevel: level,
            );
    ...
    property('compression level 22 output size <= level 1 for same input', () {
      forAll(binary(minLength: 100, maxLength: 1000), (List<int> data) async {
        if (skipPlatform) return;
        final input = Uint8List.fromList(data);
        final cli = ZstandardCLI();

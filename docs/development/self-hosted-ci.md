...
The workflow first bootstraps the Android SDK tools, then runs `flutter build apk --debug`, starts an API 30 `google_apis` `pixel_4` emulator with software acceleration, waits for the Android `package` service, and runs every file in `zstandard_android/example/integration_test/`. A missing SDK or platform tools fails before the test starts with a diagnostic message. Software emulation is slower than KVM and is given a longer job/boot timeout. Animation disabling is intentionally left off because this software image can report `boot_completed` before the Android `settings` service is ready; the tests do not depend on that optimization.

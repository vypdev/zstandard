...
2. Run `zstandard_ios/scripts/sync_zstd.sh` and `zstandard_macos/scripts/sync_zstd.sh` (from repo root) to refresh the ignored source trees used by CocoaPods. Swift Package Manager consumes the canonical source through the repository-level `Package.swift` and needs no copied iOS tree.
